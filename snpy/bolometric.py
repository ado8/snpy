...
from snpy.filters import ch, fset, standards
...
def bolometric_SED(
    ...
    if verbose:
        log("Starting bolometric calculation for %s\n" % sn.name)

    if EBVhost is None:
        EBVhost = getattr(sn, "EBVhost", None)
        if EBVhost is None:
            raise AttributeError(
                "Error:  you must either specify E(B-V) or "
    ...
    if Rv is None:
        ...
        if Rv is None:
            raise AttributeError(
                "Error:  you must either specify Rv or "
    ...
    if redlaw is None:
        ...
        if redlaw is None:
            raise AttributeError(
                "Error:  you must either specify redlaw or "
    ...
    if refband is not None:
        if refband not in bands:
            raise ValueError("refband %s is not one of your observed filters" % refband)
    ...
    if Tmax is None:
        ...
        if Tmax is None:
            raise ValueError(
                "You must supply a time of B maximum or fit a "
    ...
    if interpolate is not None:
        if interpolate == "spline":
            if verbose:
                log("   Using spline interpolation")
            for b in bands:
                if getattr(sn.data[b], "interp", None) is None:
                    raise ValueError(
                        "You asked for spline interpolation, but "
                        "filter %s has no interpolator defined" % b
                    )
        else:
            if verbose:
                log("   Using model interpolation")
            for b in bands:
                if b not in sn.model._fbands:
                    raise ValueError(
                        "You asked for model interpolation, but "
                        "filter %s was not fit with the model" % b
                    )
    else:
        if verbose:
            log("   Not using interpolation")
    ...
    if type(SED) is type(""):
        ...
        if SED in ["H3", "H", "N", "91bg"]:
            ...
            if verbose:
                log("   Using SED template '%s'" % SED)
        elif SED in standards:
            fSED = lambda x: (standards[SED].wave, standards[SED].flux)
            if verbose:
                log("   Using standards['%s'] to compute effective wavelengths" % (SED))
        else:
        ...
        try:
            ...
            raise ValueError(
                "If SED is a function, it must take single"
    ...
    if use_stretch:
        ...
        if s is None:
            if dm15 is None:
                raise ValueError(
                    "If you want to apply a stretch to the SED's, "
                    "you must fit a model that uses dm15 or st as a parameter"
                )
            if dm15 > 1.7:
                if verbose:
                    log(
                        "Warning:  dm15 > 1.7. Hsiao template is not "
                ...
                if verbose:
                    log(
                        "Warning:  dm15 < 0.7. This is a very slow "
    ...
    if verbose:
        log("   Working on data matrix with size (%d,%d)" % mags.shape)
    ...
    for i in range(len(ts)):
        ...
        if min(lam1) < wave.min() or (max(lam2) > wave.max() and not extrap_red):
            raise RuntimeError(
                "Error: your limits of integration (%.3f,%.3f) "
        ...
        if refband is None:
            ...
            filt = fset[bs[0]]
            if verbose:
                log("   Using %s as reference filter" % bs[0])
        else:
            if refband not in bs:
                if verbose:
                    log(
                        "Warning: refband %s has no observation or "
        ...
        if len(bs) == 1:
            ...
            mflux, ave_wave, pars = mangle_spectrum2(
                wave * (1 + sn.z),
                flux,
                bs,
                mags[i, masks[i]],
        ...
        mflux = (
            mflux
            * power(10, -0.4 * (mag - filt.zp))
            / filt.response(wave, mflux / (1 + sn.z), z=sn.z)
        )
        ...
        mflux, a, mask2 = deredden.unred(
            wave[mask1], mflux, EBVhost, R_V=Rv, redlaw=sn.redlaw
        )
        redlaw_mask = mask1[mask2]
        wave = wave[redlaw_mask]
        mfunc = mfunc[redlaw_mask]
        ...
        for j in range(len(i1)):
            ...
            if lam2[j] > wave.max():
                ...
                fbol[-1] += (
                    mflux[-1] * wave[-1] / 3 * (1 - power(wave[-1] / lam2[j], 3))
                )
...
def bolometric_direct(
    ...
    if SED is None:
        if verbose:
            log("   Using Vega SED to compute effective wavelengths")
    ...
    for i in range(len(ts)):
        ...
        # Deredden
        A_lamb_host = (
            array([fset[f].R(Rv=Rv, wave=wave, flux=flux, z=sn.z, redlaw=sn.redlaw) for f in bands])
            * EBVhost
        )
        A_lamb_MW = (
            array([fset[f].R(Rv=3.1, wave=wave, flux=flux, redlaw=sn.redlaw) for f in bands]) * sn.EBVgal
        )
        ...
        flam = flam * array(
            [
                fset[f].response(wave, flux, z=sn.z, photons=0)
