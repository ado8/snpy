...
class sn(object):
   ...
   """
    ...
    def __init__(self, name, source=None, ra=None, dec=None, z=0, get_spectra=False):
        ...
        if source is None:
            if ra is None or dec is None:
                if have_sql:
                    ...
                    print(
                        "Warning:  ra and/or decl not specified and no source specified."
                    )
    ...
    def choose_model(self, name, stype="st", set_restbands=True, **kwargs):
      ...
      """
        ...
        if not self.model.external_fitter:
            self.template_bands = [
                b for b in self.model.rbs if b not in ["Bs", "Vs", "Rs", "Is"]
            ]
      ...
      """
        ...
        if outfile is not None:
            ...
            for i in range(len(ret_data["MJD"])):
                data = [ret_data["MJD"][i]]
                for b in bands:
                    data += [ret_data[b][i], ret_data["e_" + b][i]]
    ...
    def color_stretch(
        self, Bband, Vband, interpolate=True, plot=False, Nboot=10, method=None, **args
    ):
      ...
      """
        ...
        if len(tBVmaxs) < Nboot:
            print(
                "Warning, {}% of iterations failed".format(
        ...
        if plot:
            ...
            plotmod.plot_sBV(
                self, t[gids], BV[gids], eBV[gids], tBVmax, stBVmax, ts, ys, eys, Tmax
            )
    ...
    def lira(
      ...
      """
        if kcorr is not None:
            warnings.warn(
                "Use of kcorr argument is deprecated. Use dokcorr " " instead",
                stacklevel=2,
            )
            dokcorr = kcorr
        ...
        t, BV, eBV, flag = self.get_color(
            Bband, Vband, dokcorr=dokcorr, interp=interpolate
        )
        ...
        if not sometrue(gids):
            raise RuntimeError(
                "Sorry, no data available between t=%f and t=%f" % (tmin, tmax)
            )
        ...
        rchisq = sum(power(BV2 - c[0] - c[1] * (t2 - 55.0), 2) * w) / (len(BV2) - 2)
        ec = ec * sqrt(rchisq)

        if B14:
            st = getattr(self, "st", 1.0)
            lira_BV = 0.78 - (0.0097 - 0.004 * (st - 1)) * (t2 - 45.0)
        else:
      ...
      """
        ...
        for band in bands:
            if band not in model_bands and band not in lc_model_bands:
                raise ValueError(
                    "Error:  filter %s has not been fit " % band
                    + "with a light-curve yet, so I cannot compute it's maximum"
                )
        N = len(bands)
        result = (
            zeros(N, dtype=float32),
            zeros(N, dtype=float32),
            zeros(N, dtype=float32),
            [""] * N,
        )
        if len(model_bands) > 0:
            mod_result = self.model.get_max(
                model_bands, restframe=restframe, deredden=deredden
            )
        for i in range(N):
            b = bands[i]
            if b in model_bands:
                ...
                for j in range(4):
                    result[j][i] = mod_result[j][mid]
            if b in lc_model_bands:
                if b in model_bands:
                    print(
                        "Warning:  both model and spline fits present, using "
                        + "spline values"
                    )
                if restframe:
                    print(
                        "Warning:  can't k-correct spline fits, you're getting "
                        + "observed maxima!"
                    )
                if deredden:
                    if band in self.Robs:
                        if type(self.Robs[band]) is type(()):
                            R = scipy.interpolate.splev(
                                self.data[b].Tmax, self.Robs[band]
                            )
                        else:
      ...
      """
        ...
        for band in bands:
            ...
            if band not in mopt:
                ...
                days = days.tolist()
                self.Ss[band], self.Ss_mask[band] = list(
                    map(
                        array,
                        kcorr.kcorr(
                            days,
                ...
                for i in range(len(self.data[band].MJD)):
                    ...
                    if wave is None:
                        ...
                        S, f = kcorr.S(
                            wave, flux, fset[band], fset[self.restbands[band]], z=self.z
                        )
    ...
    def merge_filters(self, bands=None, method="scorr"):
      ...
      Args:
        ...
        for b in bands:
            if method == "scorr" and (
                getattr(self, "Ss", None) is None or b not in self.Ss
            ):
        ...
        for rb in rbs:
            ...
            if method == "kcorr":
                mags = concatenate([self.data[b].magnitude - self.ks[b] for b in bs])
                masks = concatenate([self.data[b].mask & self.ks_mask[b] for b in bs])
            elif method == "scorr":
                mags = concatenate([self.data[b].magnitude - self.Ss[b] for b in bs])
                masks = concatenate([self.data[b].mask & self.Ss_mask[b] for b in bs])
            else:
            ...
            for b in bs:
                if method == "kcorr":
                    ...
                    del self.Ss_mask[b]
                del self.data[b]
                if self.filter_order is not None and b in self.filter_order and b != rb:
                    del self.filter_order[self.filter_order.index(b)]
                    if len(self.filter_order) == 0:
                        self.filter_order = None
                if b in self.restbands:
                    del self.restbands[b]
            ...
            # Create the LC
            self.data[rb] = lc(self, rb, MJDs, mags, emags, SNR=SNRs, sids=sids)
      ...
      """
        if use_stretch and self.k_version != "91bg":
            ...
            if dm15 is None and st is None:
                raise AttributeError(
                    "Before you can k-correct with stretch, you"
        ...
        if not mangle:
            for band in bands:
                ...
                self.ks[band], self.ks_mask[band] = list(
                    map(
                        array,
                        kcorr.kcorr(
                            days,
                ...
                if len(x) > 1:
                    self.ks_tck[band] = fit_spline.make_spline(
                        x,
                        self.ks[band],
                        x * 0 + 1,
                        k=1,
        ...
        while sometrue(less(dwaves, min_filter_sep)):
            ...
            dwaves = eff_waves[1:] - eff_waves[0:-1]
        if not self.quiet:
            print("Mangling based on filters:", mbands)
        ...
        kextrap = getattr(self, "k_extrapolate", False)
        kcorrs, mask, Rts, m_opts = kcorr.kcorr_mangle(
            t / (1 + self.z) / s,
            bands,
            mags,
            masks,
        ...
        for i in range(len(bands)):
            b = bands[i]
            self.ks_tck[b] = fit_spline.make_spline(
                res["MJD"],
                kcorrs[:, i],
                res["MJD"] * 0 + 1,
                k=1,
            ...
            self.ks_mask[b] = array(
                [
                    mask[argmin(absolute(res["MJD"] - self.data[b].MJD[j])), i]
                    for j in range(len(self.data[b].MJD))
                ]
            ).astype(bool)
            self.ks_mopts[b] = [
                m_opts[argmin(absolute(res["MJD"] - self.data[b].MJD[j]))]
                for j in range(len(self.data[b].MJD))
            ]

            self.Robs[b] = fit_spline.make_spline(
                res["MJD"],
                Rts[:, i],
                res["MJD"] * 0 + 1,
                k=1,
    ...
    def get_mangled_SED(self, band, i, normalize=True):
      ...
      """
        ...
        kextrap = getattr(self, "k_extrapolate", False)
        wave, flux = kcorr.get_SED(
            int(epoch), version=self.k_version, extrapolate=kextrap
        )
        if self.ks_mopts[band][i]:
            man_flux = mangle_spectrum.apply_mangle(
                wave, flux, **self.ks_mopts[band][i]
            )[0]
        else:
            man_flux = flux
        if not normalize:
        ...
        num = power(
            10, -0.4 * (self.data[band].magnitude[i] - self.data[band].filter.zp)
        )
    ...
    def interp_table(self, bands, use_model=False, model_float=False, dokcorr=False):
        ...
        for band in bands:
            ...
            if dokcorr:
                if band not in self.ks_tck:
                    raise RuntimeError(
                        "No k-corrections defined for %s.  Either set dokcorr=0 or run self.kcorr() first"
                        % band
                    )
                k = scipy.interpolate.splev(data["MJD"], self.ks_tck[band])
                kflag = (
                    less(data["MJD"], self.ks_tck[band][0][0])
                    & greater(data["MJD"], self.ks_tck[band][0][-1])
                ) * 8
            ...
            if use_model:
                if model_float:
                    ...
                    offset = sum((self.data[band].mag - temp) * weight) / sum(weight)
                    # error in mean offset  (MAD)
                    doffset = median(absolute(self.data[band].mag - temp - offset))
                else:
                    offset = 0
                    doffset = 0
                ...
                iids = (
                    mask
                    & greater_equal(data["MJD"], self.data[band].MJD.min())
                    & less_equal(data["MJD"], self.data[band].MJD.max())
                )
                # IDs where valid extrapolation is done
                eids = mask & (
                    less(data["MJD"], self.data[band].MJD.min())
                    | greater(data["MJD"], self.data[band].MJD.max())
                )
                flags.append(
                    where(bids, iids * 1 + eids * 2 + (~mask) * 4, bids) + kflag
                )
            else:
      ...
      """
        ...
        if not interp:
            ...
            if dokcorr:
                ...
                #  to data['MJD'].
                ids1 = argmin(
                    absolute(self.data[band1].MJD[:, newaxis] - mjd[newaxis, :]), axis=0
                )
                ids2 = argmin(
                    absolute(self.data[band2].MJD[:, newaxis] - mjd[newaxis, :]), axis=0
                )
                col = col - self.ks[band1][ids1] + self.ks[band2][ids2]
                flags = (
                    flags
                    + (
                        logical_not(self.ks_mask[band1][ids1])
                        * logical_not(self.ks_mask[band2][ids2])
                    )
                    * 8
                )
            return (mjd, col, ecol, flags)
        elif use_model and mod1 and mod2:
            MJD, ms, ems, flags = self.interp_table(
                [band1, band2], use_model=True, model_float=model_float, dokcorr=dokcorr
            )
            ...
            raise RuntimeError(
                "You asked for interpolation, but there are no"
    ...
    def getEBVgal(self, calibration="SF11"):
      ...
      """
        if self.ra is not None and self.decl is not None:
            self.EBVgal, mask = dust_getval.get_dust_RADEC(
                self.ra, self.decl, calibration=calibration
            )
    ...
    def get_distmod(self, cosmo="LambdaCDM", **kwargs):
      ...
      """
        try:
            ...
            raise ImportError(
                "Sorry, in order to compute distance modulus, "
    ...
    def summary(self, out=sys.stdout):
      ...
      """
        ...
        if self.ra:
            print("ra=%9.5f        " % (self.ra), end=" ", file=out)
        if self.decl:
            print("dec=%9.5f" % (self.decl), end=" ", file=out)
        print("", file=out)
        print("Data in the following bands:", end=" ", file=out)
        for band in self.data:
            print(band + ", ", end=" ", file=out)
        print("", file=out)
    ...
    def dump_lc(
        self, epoch=0, tmin=-10, tmax=70, k_correct=0, s_correct=False, mw_correct=0
    ):
      ...
      """
        ...
        for filt in list(self.data.keys()):
            ...
            if s_correct:
                print(
                    "# NOTE  Data have been S-corrected from %s to %s"
                    % (filt, self.restbands[filt]),
                    file=f,
                )
            if mw_correct:
            ...
            if filt in self.model._fbands:
                ...
                if k_correct and filt in self.ks_tck:
                    ...
                    mids = argmin(
                        absolute(
                            ts[:, newaxis] - self.data[filt].MJD[newaxis, :] + self.Tmax
                        ),
                        axis=1,
                    )
                    ks_mask = (
                        self.ks_mask[filt][mids]
                        * greater_equal(ts, -19)
                        * less_equal(ts, 70)
                    )
                    mask = mask * ks_mask
                ...
                for i in range(len(ts)):
                    print(
                        "%.1f, %.3f %.3f" % (ts[i] + self.Tmax - toff, ms[i], e_ms[i]),
                        file=f,
                    )
                f.close()
    ...
    def to_txt(self, filename, mask=True):
      ...
      """
        ...
        for filt in list(self.data.keys()):
            ...
            for i in range(len(self.data[filt].mag)):
                # mask out bad data
                if mask and not self.data[filt].mask[i]:
                    continue
                print(
                    "%.2f  %.3f  %.3f"
                    % (
                        self.data[filt].MJD[i],
                        self.data[filt].mag[i],
                        self.data[filt].e_mag[i],
                    ),
                    file=f,
                )
      ...
      """
        ...
        if Tmax is None:
            if self.Tmax > 1.0:
                ...
                diffs = [
                    abs(self.data[band].filter.ave_wave - fset["B"].ave_wave)
        ...
        for filt in bands:
            ...
            for i in range(len(l.mag)):
                if flux:
                    ...
                    fout.write(
                        fmt.format(l.MJD[i], l.mag[i], l.e_mag[i], inst, f, magsys)
                    )
        fout.close()
        return trans
        ...
        if have_sql:
            ...
            for filt in data:
                ...
                self.data[filt] = lc(
                    ...
                    K=K,
                    SNR=SNR,
                    sids=zeros(d["t"].shape, dtype=int),
                )
    ...
    def lc_offsets(self, min_off=0.5):
      ...
      """
        ...
        x, y, ey = regularize(
            self.data[filt].MJD, self.data[filt].mag, self.data[filt].e_mag
        )
        if len(x) < 3:
            mn = y.mean()
            f = lambda x: mn
        else:
            f = interp1d(x, y, bounds_error=False, fill_value=self.data[filt].mag.max())
    ...
    def fit(
      ...
      """
        ...
        if bands is None:
            ...
            bands = [
                b for b in list(self.data.keys()) if self.restbands[b] in self.model.rbs
            ]
        ...
        for filt in bands:
            if self.restbands[filt] not in self.model.rbs:
                raise AttributeError(
                    "Error:  filter %s is not supported by this model"
                    % self.restbands[filt]
      ...
      * specify a function that takes a single argument:  the value of the
      ...
      """
        ...
        if len(self.model._fbands) == 0:
            raise AttributeError(
                "In order to fit with the MCMC sampler, you need to do \
        ...
        sampler, vinfo, p0 = snemcee.generateSampler(
            self, bands, Nwalkers, threads, tracefile, **args
        )
        if verbose:
        ...
        for par in vinfo:
            if (
                type(vinfo[par]) is type({})
                and "index" in vinfo[par]
        ...
        if tracefile is not None:
            d = dict(
                samples=samples, vinfo=vinfo, pos=pos, prob=prob, state=state, pars=pars
            )
    ...
    def plot_color(
      ...
      """
        ...
        return plotmod.plot_color(
            self, f1, f2, epoch, deredden, interp, dokcorr, outfile, clear
        )
    ...
    def compute_w(
      ...
      """
        ...
        if dokcorr:
            if band1 not in self.ks or band2 not in self.ks or band3 not in self.ks:
                raise ValueError(
                    "Not all requested bands have k-corrections.  "
        ...
        if not interp:
            ...
            gids = less(data[band1], 90) & less(data[band2], 90) & less(data[band3], 90)
            if dokcorr:
            ...
            mjd = data["MJD"]
            m1 = data[band1][gids] - k1
            em1 = data["e_" + band1][gids]
            m2 = data[band2][gids] - k2
            em2 = data["e_" + band2][gids]
            m3 = data[band3][gids] - k3
            em3 = data["e_" + band3][gids]
      ...
      """
        ...
        if outfile is not None:
            ...
            if len(shape(res["boloflux"])) == 2:
                ...
                fout.write(
                    "#\n#%6s  %9s  %8s %8s %s\n"
                    % ("t", "Lbol", "lam1", "lam2", "filters")
                )
                for i in range(len(res["epochs"])):
                    fout.write(
                        "%7.2f  %6.3f  %8.1f %8.1f %s\n"
                        % (
                            res["epochs"][i],
                            res["boloflux"][i] / 1e42,
                            limits[i][0],
    ...
    def closest_band(self, band, tempbands=None, lowz=0.15):
      ...
      regardless of whether another band is closer."""
        ...
        if self.z < lowz and band in tempbands:
            return band

        resps = []
        # normalize responses to the area under the filter response curve
        norm = fset[band].response(
            fset[band].wave, fset[band].wave * 0.0 + 1.0, z=0, zeropad=1, photons=0
        )
        for temp in tempbands:
            norm2 = fset[temp].response(
                fset[temp].wave, fset[temp].wave * 0.0 + 1.0, z=0, zeropad=1, photons=0
            )
            resps.append(
                fset[band].response(
                    fset[temp].wave, fset[temp].resp, z=self.z, zeropad=1, photons=0
                )
                * norm
                / norm2
            )
        ...
        if max(resps) <= 0:
            ...
            dists = absolute(
                array([fset[temp].ave_wave - fset[band].ave_wave for temp in tempbands])
            )
...
def import_lc(file):
   ...
   """
    ...
    if len(fields) != 4:
        raise RuntimeError(
            "first line of %s must have 4 " + "fields:  name, redshift, RA, DEC"
        )
    name = fields[0]
    try:
        ...
        raise RuntimeError(
            "z, ra and dec must be floats " + " (ra/dec in decimal degrees)"
        )
...
def get_sn(str, sql=None, **kw):
   ...
   are sent as options to the sql module."""
    ...
    elif str.find("http://") == 0 or str.find("https://") == 0 or str.find("osc:") == 0:
        from . import get_osc
...
def check_version():
    ...
    try:
        u = urllib.urlopen(
            "ftp://ftp.obs.carnegiescience.edu/pub/cburns/snpy/latest", timeout=1
        )
    except:
        return None, None
    if not u:
