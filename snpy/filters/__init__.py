...
class spectrum:
    ...
    def __init__(
        ...
        if filename is not None and load == 1:
            self.read()
    ...
    def read(self):
        ...
        if self.file is not None:
            f = open(self.file)
            lines = f.readlines()
            self.wave_data = num.array(
                [float(line.split()[0]) for line in lines if line[0] != "#"]
            )
            self.resp_data = num.array(
                [float(line.split()[1]) for line in lines if line[0] != "#"]
            )
            f.close()
...
class filter(spectrum):
    ...
    def eval(self, wave):
        ...
        if interp_method == "spline":
            ...
            if self.mint is None:
                self.mint = scipy.interpolate.interp1d(
                    self.wave, self.resp, kind=interp_method
                )
            fresp_int = self.mint(trim_wave)

        fresp_int = num.where(num.less(wave, self.wave.min()), num.nan, fresp_int)
        fresp_int = num.where(num.greater(wave, self.wave.max()), num.nan, fresp_int)
        return fresp_int
    ...
    def response(self, specwave, flux=None, z=0, zeropad=0, photons=1):
        ...
        if integ_method == "simpsons":
            ...
            result = (
                (trim_wave[-1] - trim_wave[0]) / (len(trim_wave) - 1) * sum(integrand)
            )
    ...
    def synth_abmag(self, specwave, flux=None, z=0, zeropad=0):
        ...
        numer = self.response(specwave, flux=flux, z=z, zeropad=zeropad, photons=1) * ch
        ...
        if not isinstance(specwave, spectrum):
            ...
            denom = self.response(specwave, 3631 * 1.0e-23 * c / specwave, photons=0)
        else:
            wave = specwave.wave
            denom = self.response(wave, 3631 * 1.0e-23 * c / wave, photons=0)
    ...
    def mag2flux(self, mag, specwave=None, flux=None, z=0):
        ...
        if specwave is None:
            ...
            if flux is None:
                raise TypeError(
                    "specwave must either be a spectrum instance or "
    ...
    def eff_wave(self, specwave, flux=None, z=0, zeropad=0):
        ...
        numer = self.response(
            s_wave, flux=s_flux * s_wave, z=z, zeropad=zeropad, photons=0
        )
        denom = self.response(s_wave, flux=s_flux, z=z, zeropad=zeropad, photons=0)
    ...
    def R(
        ...
        redf, _, redlaw_mask = unred(wave, flux, -EBV, Rv, z, redlaw=redlaw)
        fluxr = self.response(wave[redlaw_mask], redf, z, photons=1)
        flux0 = self.response(wave[redlaw_mask], flux[redlaw_mask], z, photons=1)
        if flux0 <= 0:
            return num.nan
        ...
        for syst in list(self.systems.values()):
            for sed in list(syst.SEDs.values()):
                if sed.name in self.spectra:
                    print("Warning!  Encountered multiple filter IDs for %s" % (sed))
            ...
            for tel in list(obs.telescopes.values()):
                for filt in list(tel.filters.values()):
                    if filt.name in self.filters:
                        print(
                            "Warning!  Encountered multiple filter IDs for %s" % (filt)
                        )
...
dirs = [
    dir
    for dir in dirs
    if os.path.isdir(dir) and os.path.isfile(os.path.join(dir, "standards.dat"))
]
...
for dir in dirs:
    ...
    for line in lines:
        if line[0] == "#":
            continue
        l = line.split()
        standards[sname].add_SED(
            spectrum(
                l[0], os.path.join(stand_base, sname, l[1]), " ".join(l[3:]), load=0
            )
        )
...
for obs in obsdirs:
    ...
    teldirs = [
        dir
        for dir in teldirs
        if os.path.isdir(dir) and os.path.isfile(os.path.join(dir, "filters.dat"))
    ]
    for dir in teldirs:
            ...
            if l[2].find("=") >= 0:
                ...
                if std in standards:
                    try:
                        ...
                        raise ValueError(
                            "Could not convert standard magnitude for filter %s" % l[0]
                        )
                    newf = filter(l[0], os.path.join(dir, l[1]), 0.0, " ".join(l[3:]))
                    newf.zp = newf.compute_zpt(standards[std], m, zeropad=1)
                ...
                fset.observatories[obs_name].telescopes[tel_name].add_filter(
                    filter(l[0], os.path.join(dir, l[1]), float(l[2]), " ".join(l[3:]))
                )
        f.close()
fset.cache_filters()
