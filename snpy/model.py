...
from scipy.optimize import brent, leastsq

from snpy import kcorr, mlcs_utils, salt_utils, ubertemp
...
def _quniform_prior(p, pmin, pmax, sigma):
    ...
    if pmin < p < pmax:
        return norm
    if p <= pmin:
        return norm * exp(-0.5 * (p - pmin) ** 2 / sigma ** 2)
    if p >= pmax:
        return norm * exp(-0.5 * (p - pmax) ** 2 / sigma ** 2)
...
class model:
   ...
   like K-corrections and reddening coefficients."""
    ...
    def kcorr(self, band, t):
      ...
      """
        ...
        if band in self.parent.ks_tck:
            ...
            mids = argmin(
                absolute(
                    t[:, newaxis] - self.parent.data[band].MJD[newaxis, :] + self.Tmax
                ),
                axis=1,
            )
            ...
            mask2 = (
                self.parent.ks_mask[band][mids]
                * greater_equal(t / ks_st, -19)
                * less_equal(t / ks_st, 70)
            )
        else:
            K = 0 * t
        ...
        if band in self.parent.Robs:
            if type(self.parent.Robs[band]) is type(()):
                R = scipy.interpolate.splev(
                    t + self.parent.Tmax, self.parent.Robs[band]
                )
            else:
    ...
    def fit(self, bands, epsfcn=0, **args):
      ...
      """
        self.args = args.copy()
        if debug:
            print("model.fit() called with args = ", args)
        ...
        for key in list(args.keys()):
            if key in self.parameters:
                ...
                try:
                    ...
                    raise ValueError(
                        "You are trying to hold %s fixed, but with an illegal value or type: %s"
                        % (key, str(args[key]))
                    )
        ...
        for i in range(len(pars)):
            if pars[i] is None:
                ...
                if debug:
                    print("Initial guess for %s = %f" % (self._free[i], pars[i]))
        ...
        pars, C, self.info, self.mesg, self.ier = leastsq(
            self._wrap_model, pars, (bands, error), full_output=1
        )
        if self.ier > 4:
            print(self.mesg)
        ...
        for p in self.parameters:
            self.errors[p] = self._extra_error(p)
   ...
   """
    ...
    def _wrap_model(self, pars, bands, error):
        ...
        for band in bands:
            if debug:
                print(">>> calling model member function")
        ...
        if debug:
            print("  weighted resids = ", resids_list)
        # N = len(W[m])
   ...
   extinction is taken from the SN object (default 3.1)."""
    ...
    def setup(self):
        if "EBVhost" not in self.args:
            if len(self._fbands) < 2:
                raise RuntimeError(
                    "Error:  to solve for EBVhost, you need to fit more than one filter"
                )
    ...
    def guess(self, param):
        ...
        if param == "DM":
            ...
            if s.zcmb < 2e-4:
                print(
                    "Warning: SN CMB redshift is very small or negative. "
    ...
    def get_max(self, bands, restframe=0, deredden=0):
        ...
        for band in bands:
            ...
            x0 = brent(
                lambda x: self.template.eval(rband, x, gen=self.gen)[0],
                brack=(0.0, 5.0),
            )
            ...
            if not restframe and band in self.parent.ks_tck:
                ...
                mmax = mmax + scipy.interpolate.splev(
                    Tmaxs[-1], self.parent.ks_tck[band]
                )
            if not deredden:
                if self.do_Robs:
                    Robs = kcorr.R_obs(
    ...
    def MMax(self, band, calibration=6):
      ...
      fit (1-6) in Folatelli et al. (2009), table 9"""
        if band == "Bs":
            ...
            return (
                self.M0[calibration]
                + (self.dm15 - 1.1) * self.b[calibration]
                - self.colors[band]
                - self.color_slopes[band] * (self.dm15 - 1.1)
            )
        else:
            return -19.0
...
def read_table(file):
    ...
    for line in lines:
        ...
        f = line[1]
        a[id][f] = float(line[2])
        ea[id][f] = float(line[3])
        b[id][f] = float(line[4])
        eb[id][f] = float(line[5])
        c[id][f] = float(line[6])
        ec[id][f] = float(line[7])
        Rv[id] = float(line[8])
        eRv[id] = float(line[9])
    ...
    def MMax(self, band, calibration=1):
      ...
      fit (1-6) in Folatelli et al. (2009), table 9"""
        ...
        if self.stype in ["st"]:
            ...
            delta = self.dm15 - 1.1
        return (
            self.a[calibration][band]
            + self.b[calibration][band] * delta
    ...
    def systematics(self, calibration=1, include_Ho=False):
      ...
      If no estimate is available, return None for that paramter."""
        ...
        for band in self._fbands:
            ...
            syst_DM.append(
                power(dst * self.eb[calibration][rb], 2)
                + power(self.EBVhost * dRobs, 2)
                + power(self.ea[calibration][rb], 2)
                + power(  # power(2.17*velerr/(3e5*self.parent.z),2) +\
                    self.sigSN[calibration][rb], 2
                )
            )
        syst_DM = array(syst_DM)
        weights = array(weights)
...
class max_model(model):
   ...
   of R_V)."""
    ...
    def __init__(self, parent, stype="dm15"):
        ...
        # AD 20230501: these seem to be from Table 9 of Folatelli et al 2010,
        # but the values are off by up to 0.02
        self.M0s = {
            "u": -18.82,
            "B": -19.11,
            "V": -19.12,
    ...
    def __call__(self, band, t, extrap=False):
        if debug:
            print(">>>   Now in max_model")
        if debug:
            print(">>>> setting shape parameter to ", self.parameters[self.stype])
        self.template.mktemplate(self.parameters[self.stype])
        t = t - self.Tmax
        rband = self.parent.restbands[band]
    ...
    def get_max(self, bands, restframe=0, deredden=0):
        ...
        for band in bands:
            ...
            if rband + "max" not in self.parameters:
                raise ValueError(
                    "Trying to find max of %s, but haven't solved for %s"
                    % (band, rband + "max")
                )
            ...
            if not deredden:
                if band in self.parent.Robs:
                    if type(self.parent.Robs[band]) is type(()):
                        R = scipy.interpolate.splev(
                            x0 + self.Tmax, self.parent.Robs[band]
                        )
                    else:
                        R = self.parent.Robs[band]
                else:
    ...
    def _extra_error(self, parameter):
      ...
      observations start after Tmax."""
        days = array([0.0, 5.0, 10.0, 15.0, 20.0])
        errors = {
            "dm15": array([0.00, 0.00, 0.03, 0.03, 0.03]),
    ...
    def guess(self, param):
        ...
        if param[0] == "T":
            ...
            for f in self._fbands:
                if self.parent.restbands[f] == rfilt:
                    filt = f
        ...
        for band in bands:
            ...
            if not deredden:
                if band in self.parent.Robs:
                    if type(self.parent.Robs[band]) is type(()):
                        R = scipy.interpolate.splev(
                            x0 + self.parameters["T" + rband + "max"],
   ...
   value of Rv and EBVhost to get predicted maximum magnitudes."""
    ...
    def guess(self, param):
        ...
        if param == "Bmax":
            if s.zcmb < 2e-4:
                print(
                    "Warning: SN CMB redshfit is very small or negative. "
    ...
    def __call__(self, band, t, extrap=False):
        ...
        # Host reddening:
        Ahost = kcorr.A_obs(
            rband, 0, 0, self.EBVhost, 0, self.Rv, self.parent.Rv_gal, "H3", extrap
        )
        temp = temp + Ahost + Rmw * self.parent.EBVgal
        temp = (
            temp
            + self.Bmax
            + self.MMax(rband, self.calibration)
    ...
    def MMax(self, band, calibration=1):
      ...
      fit (1-6) in Folatelli et al. (2009), table 9"""
        ...
        if band in ["u", "B", "V", "g", "r", "i", "Y", "J", "H"]:
            return (
                self.M0[band][calibration]
                + (self.dm15 - 1.1) * self.b[band][calibration]
            )
        else:
            return -19.0
...
class color_model(model):
   ...
   """
    ...
    def __init__(self, parent, stype="st", normfilter="B"):
        ...
        self.parameters = {"st": None, "EBVhost": None, "Rv": None, "Tmax": None}
    ...
    def setup(self):
        ...
        for f in self.parent.data:
            if self.parent.restbands[f] == self.normfilter:
                nf.append(f)
        if len(nf) == 0:
            raise RuntimeError(
                "Error, to use color-model, you must have a B rest-frame observation"
            )
        self.nf = nf
        ...
        if six.PY3:
            ...
            cdata = pickle.load(f)
        f.close()
        if self.redlaw not in cdata or self.rvprior not in cdata[self.redlaw]:
            raise ValueError(
                "Intrinsic colors for %s prior and %s reddening law not found"
    ...
    def prior(self, param, value):
        if param == "Rv":
            ...
            if self.rvprior == "uniform":
                ...
                return (
                    gconst
                    - 0.5 * power(value - self.mu_i[id], 2) * self.tau_i[id]
                ...
                p = sum(
                    pi_i
                    * sqrt(tau_i / 2 / pi)
                    * exp(-0.5 * power(value - self.mu_i, 2) * self.tau_i)
                )
                return log(p)
...
class SALT_model(model):
   ...
   resulting parameters and fit back into SNooPy."""
    ...
    def __call__(self, band, t, extrap=False):
        ...
        mask = greater_equal(t, self._tck[band][0].min()) * less_equal(
            t, self._tck[band][0].max()
        )
    ...
    def setup(self):
        ...
        if self.bindir is not None:
            if os.path.isfile(os.path.join(self.bindir, "snfit")):
                ...
                raise ValueError("Cannot find snfit at location {}".format(self.bindir))
        else:
    ...
    def _runsalt(self, verbose=False):
        ...
        self._trans = self.parent.to_salt(
            self._fbands, outfile=self.saltfile, stock=(not self.cspsalt)
        )
    ...
    def guess(self, param):
        ...
        if param == "Bmax":
            fs = list(s.data.keys())
            idx = argmin(absolute(array([fset[f].ave_wave for f in fs]) - 4378.0))
    ...
    def fit(self, bands, epsfcn=0, **args):
        ...
        for key in list(args.keys()):
            if key in self.parameters:
                ...
                if key == "DM":
                    print("Warning:  DM is a derived quantity, so cannot be held fixed")
                    continue
                try:
                    testvalue = 1.0 * args[key]
                except TypeError:
        ...
        self._lcs = salt_utils.parse_lc(
            self.modfile, stock=(not self.cspsalt), trans=self._trans
        )
        ...
        self.C["DM"]["X0"] = 0
        for key in self.C["DM"]:
            self.C[key]["DM"] = self.C["DM"][key]
...
class MLCS_model(model):
   ...
   resulting parameters and fit back into SNooPy."""
    ...
    def setup(self):
        ...
        if self.idl is not None:
            if not os.path.isfile(self.idl):
                raise ValueError("Cannot find IDL at location {}".format(self.idl))
        else:
            if "IDL_DIR" in os.environ:
                ...
                p = subprocess.Popen(
                    ["which", "idl"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
                )
                so, se = p.communicate()
                if p.returncode != 0:
    ...
    def _runmlcs(self, verbose=False):
        ...
        self._trans = self.parent.to_mlcs(
            self._fbands,
            sninfo=self.sninfo,
            outfile=self.outfile,
            prior=self.prior,
            vector=self.vector,
            spec_sample=self.spec_sample,
            stock=self.stock,
        )
        ...
        # build up the command line
        if verbose:
            print("Changing dir to {}".format(self.workdir))
    ...
    def fit(self, bands, epsfcn=0, **args):
        ...
        self._lcs = mlcs_utils.parse_lc(
            self.fitdir, stock=self.stock, trans=self._trans
        )
